    ...
    public class HotelController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateHotel(int id, [FromForm] HotelDto hotelDto, IFormFile? image)
        {
            try
            {
                ...
                hotel.Address = hotelDto.Address ?? string.Empty;
                hotel.StarRating = hotelDto.StarRating;
                hotel.DistanceToHaram = hotelDto.DistanceToHaram;
                hotel.Description = hotelDto.Description ?? string.Empty;
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, ApiResponse<HotelDto>.ErrorResponse($"Error updating hotel: {innerMessage}"));
            }
        }
        ...
        /// <summary>
        /// DEBUG: Get ALL hotels without any filter to verify database
        /// </summary>
        [HttpGet("DebugGetAll")]
        public async Task<IActionResult> DebugGetAll()
        {
            try
            {
                ...
                var activeHotels = allHotels.Where(h => h.IsActive).ToList();
                
                return Ok(new {
                    TotalInDb = allHotels.Count(),
                    ActiveCount = activeHotels.Count,
                    Hotels = allHotels.Select(h => new {
                        h.HotelId,
                        h.Name,
                        City = h.HotelCity.ToString(),
                        h.IsActive
                    })
                });
            }
