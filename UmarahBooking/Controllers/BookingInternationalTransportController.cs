        ...
        public async Task<IActionResult> BookInternationalTransport([FromBody] TransportBookingDto dto)
        {
            try
            {
                if (!ModelState.IsValid)
                    return BadRequest(ApiResponse<TransportBookingDto>.ErrorResponse("Invalid booking data"));
                ...
                {
                    return BadRequest(ApiResponse<TransportBookingDto>.ErrorResponse("User not found or not logged in"));
                }

                // 2️⃣ Call Service to complete booking
                var bookingResult = await _bookingService.BookInternationalTransportAsync(userId, dto);

                return Ok(ApiResponse<TransportBookingDto>.SuccessResponse(null, "Transport booked successfully"));
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning(ex, "Transport booking validation failed");
                return BadRequest(ApiResponse<TransportBookingDto>.ErrorResponse(ex.Message));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while booking transport");
                return StatusCode(500, ApiResponse<TransportBookingDto>.ErrorResponse(
