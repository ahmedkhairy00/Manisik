...
namespace UmarahBooking.Core.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendEmailWithAttachmentsAsync(string to, string subject, string content, List<EmailAttachment> attachments)
        {
            try
            {
                var smtpSection = _configuration.GetSection("Smtp");
                ...
                await smtp.SendMailAsync(mail);
                _logger.LogInformation("Email with {AttachmentCount} attachments sent successfully to {To}", attachments.Count, to);
            }
            catch (Exception ex)
            {
        ...
        public async Task SendBookingConfirmedWithDocumentsAsync(string to, string bookingNumber, string travelerName, string bookingType, List<EmailAttachment> attachments)
        {
            var subject = $"Your {bookingType} Booking #{bookingNumber} is Confirmed - Documents Attached";
            var tripIcon = bookingType.ToLower() == "hajj" ? "🕋" : "🕌";

            var body = GetHtmlTemplate($"Booking Confirmed - {bookingType}",
$@"<p style='font-size: 18px; margin: 0 0 20px 0;'>Dear <strong>{travelerName}</strong>,</p>
<p style='font-size: 16px; line-height: 1.6; margin: 0 0 20px 0;'>Great news! Your booking has been confirmed. We're excited to be part of your spiritual journey!</p>
<div style='margin: 30px 0; padding: 25px; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 12px; border-left: 4px solid #10B981;'>
...
<span style='display: inline-block; padding: 10px 25px; background-color: #10B981; color: white; border-radius: 25px; font-size: 16px; font-weight: 600;'>
✅ Confirmed
</span>
</div>
</div>
...
<h3 style='margin: 0 0 10px 0; color: #166534;'>📎 Your Documents Are Attached</h3>
<p style='margin: 0; color: #15803d;'>Please find attached your <strong>Visa</strong> and <strong>E-Ticket</strong> documents. Keep these safe for your journey!</p>
</div>
<p style='font-size: 15px; color: #6b7280; margin: 0 0 20px 0;'>You can also view the full details of your booking in your dashboard.</p>
